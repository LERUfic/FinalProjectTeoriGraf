...
public class Graph : MonoBehaviour
{
    ...
    public virtual Path GetShortestPath(Nodes start, Nodes end)
    {
        ...
        Dictionary<Nodes, Nodes> previous = new Dictionary<Nodes, Nodes>();

        // The calculated distances, set all to Infinity at start, except the start Node
        Dictionary<Nodes, float> distances = new Dictionary<Nodes, float>();
        ...
        while (unvisited.Count != 0)
        {
            ...
            {
                Nodes neighbor = current.connections[i];
